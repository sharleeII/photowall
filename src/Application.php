...
class Application extends BaseApplication
{
    ...
    public function middleware(MiddlewareQueue $middlewareQueue): MiddlewareQueue
    {
        $middlewareQueue
            ...
            // Skipped for:
            //   /e/*     — guest endpoints (QR-driven, no session expected)
            //   /admin/* — protected by AdminAuthMiddleware + shared-secret cookie instead
            ->add((new CsrfProtectionMiddleware([
                'httponly' => true,
            ]))->skipCheckCallback(function (ServerRequest $request): bool {
                $path = $request->getUri()->getPath();

                return str_starts_with($path, '/e/') || str_starts_with($path, '/admin');
            }))
